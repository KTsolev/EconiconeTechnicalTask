...
describe('todos actions tests', () => {
  ...
  it('should dispatch addTodo', () => {
    ...
    expectedAction = [{type: 'todod/addTodo', payload: todos[1]}];
    expect(action).toEqual(expectedAction);
  });
  it('should dispatch removeTodo', () => {
    store.dispatch(removeTodo(todos[2].id));
    action = store.getActions();
    expectedAction = [{type: 'todod/removeTodo', payload: todos[2].id}];
    expect(action).toEqual(expectedAction);
  });
  it('should dispatch completeTodo', () => {
    ...
    action = store.getActions();
    expectedAction = [
      {type: 'todod/completeTodo', payload: {id: todos[4].id, value: true}},
    ];
    expect(action).toEqual(expectedAction);
  });
  it('should dispatch editTodo', () => {
    ...
    action = store.getActions();
    expectedAction = [{type: 'todod/editTodo', payload: todos[4].id}];
    expect(action).toEqual(expectedAction);
  });
});
